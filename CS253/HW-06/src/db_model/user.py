# -*- coding: utf-8 -*-

import re

from google.appengine.ext import db

import utils.hashing as hashing

class User(db.Model):
    username = db.StringProperty(required = True)
    password = db.StringProperty(required = True)
    email = db.StringProperty(required = False)
    created = db.DateTimeProperty(auto_now_add= True)
    last_modified = db.DateTimeProperty(auto_now= True)
    
    VAL_ERROR_INVALID_USERNAME = 1
    VAL_ERROR_INVALID_PASSWORD = 2
    VAL_ERROR_PASSWORDS_MISMATCH = 3
    VAL_ERROR_INVALID_EMAIL = 4
    VAL_ERROR_USER_EXIST = 5
    
    @staticmethod
    def create_user(username, password, verify_password, email):
        val_errors = set()
        User.valid_username(username, val_errors)
        User.valid_password(password, val_errors)
        User.valid_verify(password, verify_password, val_errors)
        User.valid_email(email, val_errors)
        
        if val_errors:
            return None, val_errors
        else:
            return User(username = username, password = '%s' % (hashing.make_pw_hash(username, password)), email = email), None
    
    # Validations
    @staticmethod
    def valid_username(username, val_errors):
        user_validation_re = re.compile(r"^[a-zA-Z0-9_-]{3,20}$")
        if not(user_validation_re.match(username)):
            val_errors.add(User.VAL_ERROR_INVALID_USERNAME)
            return False
        return User.valid_user_exist(username, val_errors)
        
    @staticmethod
    def valid_password(password, val_errors):
        password_validation_re = re.compile(r"^.{3,20}$")
        if not (password_validation_re.match(password)):
            val_errors.add(User.VAL_ERROR_INVALID_PASSWORD)
            return False
        return True

    @staticmethod
    def valid_verify(password, verify, val_errors):
        if password != verify:
            val_errors.add(User.VAL_ERROR_PASSWORDS_MISMATCH)
            return False
        return True

    @staticmethod
    def valid_email(email, val_errors):
        email_validation_re = re.compile(r"^[\S]+@[\S]+\.[\S]+$")
        if email and not(email_validation_re.match(email)):
            val_errors.add(User.VAL_ERROR_INVALID_EMAIL)
            return False
        return True
    
    @staticmethod
    def valid_user_exist(username, val_errors):
        users = db.GqlQuery('SELECT * FROM User WHERE username=:username', username=username)
        if users.count() != 0:
            val_errors.add(User.VAL_ERROR_USER_EXIST)
            return False
        return True
    
    @staticmethod
    def get_verified_user(username, password):
        query = User.gql('WHERE username=:username', username=username)
        user = query.get()
        if user:
            if hashing.valid_pw(username, password, user.password):
                return user
        
